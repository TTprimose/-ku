            ...
            <div class="conent_box">
                ...
                <el-form style='margin:0' >
                    ...
                    <el-form-item class="good_title">
                        ...
                        <el-checkbox @change="checkboxChangeEvent" class="position" v-model="form.expirationDateStatus"><span style="font-size:10px">有无保质期</span></el-checkbox>
                    </el-form-item>
                    <!-- 信息内容 -->
                        ...
                        <el-form ref="addGoodsForm" :model="form" label-width="100px" :rules="rules">
                            ...
                            <el-form-item style="margin-left:-100px;margin-bottom:25px">
                                <div style="display:flex">
                                    ...
                                    <el-form-item ref="addGoodsExpReset" :required="requshow" label="保质期" prop='expirationDateName' style="margin-left:70px">
                                        ...
                                        <div v-for="(item, index) in skuProperty" :key="index">
                                            ...
                                            <div class="add_small_standard" >
                                                <el-form-item label="规格名" style="margin-left:-20px" >
                                                    <el-select v-model="item.skuPropertyName" @change="changeSku(item, index, skuList)" placeholder="请选择规格名" size='small' style="width:138px">
                                                        <el-option
                                            ...
                                            <el-form-item label='规格值' style="margin-left:-10px;margin-top:5px" >
                                                <span v-for="(itemName, i) in item.itemSkuPropertyValueDTOS" :key='i'>
                                                    <el-select @change="changeSKUname(item, index)" placeholder="请选择规格值" size='small'  v-model="itemName.skuPropertyValueName" style="width:138px;margin-right:20px">
                                                        <el-option
                                    ...
                                    <div class="add_standard">
                                        <div class="add_small_standard add_standard">
                                            <el-form-item  label="类型名" label-width="70px">
                                                <el-select v-model="typesname"  placeholder="选择类型名" size='small'  @change='gettypevalue()'>
                                                    <el-option
                                        ...
                                        <div style="padding:10px;display:flex" class="supply_check">
                                            <el-form-item label="类型值" label-width="70px">
                                                <span style="position:relative;margin-right:20px" v-for="(items, index) in typesvalue" :key="index" >
                                                    <el-select v-model="items.id" placeholder="选择类型值" size='small' @change="typechange(index)" >
                                                        <el-option
    ...
    methods:{
        ...
        updateShowArr() {
            ...
            if (this.allList_2.length > 0) {
                for (let i = 0; i < this.allList_0.length; i++) {
                    for (let j = 0; j < this.allList_1.length; j++) {
                        for (let k = 0; k < this.allList_2.length; k++) {
                            this.showSkuArr.push([this.allList_0[i], this.allList_1[j], this.allList_2[k]])
                        }
                    }
                }
                ...
                for (let i = 0; i < this.allList_0.length; i++) {
                    for (let j = 0; j < this.allList_1.length; j++) {
                        this.showSkuArr.push([this.allList_0[i], this.allList_1[j]])
                    }
                }
            } else if (this.allList_0.length > 0) {
                for (let i = 0; i < this.allList_0.length; i++) {
                    this.showSkuArr.push([this.allList_0[i]])
                }
            }
        },
        // 修改规格值
        ...
        gettypevalue() {
            ...
            api.getitemsupplyPropertyunitId(this.typesname.toString()).then((response)=>{
                // console.log(response)
